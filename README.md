# こたつLLMライセンス（KLL）テンプレートリポジトリ

> **正式名称:** こたつLLMライセンス
> **補助英語名:** Kotatsu LLM License
> **略称:** KLL
...
このリポジトリは、LLM（大規模言語モデル）またはその他の自動処理によって生成されたリポジトリ・コンテンツを対象とする、**こたつLLMライセンス（KLL）** の雛形文書セットを提供します。
...
本リポジトリで提供する文書は、LLM生成コンテンツに関する宣言文書のテンプレートであり、法的助言ではありません。具体的な事案への適用可能性については、必要に応じて専門家に確認してください。
...
- 生成物の具体的な表現について、人間は創作的な執筆・選択・配置・編集・監修を行っていない。
...
```
templates/
  LICENSE.ja.md          ← 日本語正文（ライセンス本体）
  LICENSE.en.md          ← 英語参考訳
  NO_AUTHORSHIP.ja.md    ← 非著作者性の事実前提の説明（日本語）
...
- `LICENSE` または `LICENSES/KLL.md` — ライセンス本体（`templates/LICENSE.ja.md` を元にする）
- `NO_AUTHORSHIP.md` — 非著作者性の事実前提の説明（`templates/NO_AUTHORSHIP.ja.md` を元にする）
...
This repository provides a clean-room template document set — the **Kotatsu LLM License (KLL)** (こたつLLMライセンス) — for repositories and contents generated entirely by LLMs or automated processing, where no human creative authorship exists in the concrete expression of the output.

**Key points:**

- The authoritative language of the template documents is **Japanese** (the official name is こたつLLMライセンス; "Kotatsu LLM License" / "KLL" are the English auxiliary name and abbreviation).
...
- The documents in this repository are templates, **not legal advice**. Consult a qualified professional for specific situations.
...
- Third-party rights (copyright, trademark, likeness, etc.) are outside the scope of this License and require separate consideration.
